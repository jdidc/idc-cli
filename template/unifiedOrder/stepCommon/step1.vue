<template>
    <div class="col-sm-12 mt40">
        <!-- start step 1 -->
        <div class="row step1" style="margin-top: 20px;" v-for="(pitem, i) in arrOrderType" :key="i">
            <h3 class="platform">
                {{pitem.title}}
            </h3>

            <div @click="chooseClick(i,idx)" class="col-sm-6 col-md-4 col-lg-3" v-for="(item, idx) in pitem.order_type" :key="idx">
                <Card class="item text-center">
                    <div class="left">
                        <i :class="renderIcon(item.title)"></i>
...
export default {
  ...
  methods: {
    ...
    renderIcon(title) {
      ...
      switch (title) {
        case '故障提报单':
          classes = 'fa fa-volume-control-phone';
          break;

        case '开机工单':
          classes = 'fa fa-play-circle-o';
          break;

        case '关机工单':
          classes = 'fa fa-stop-circle';
          break;
